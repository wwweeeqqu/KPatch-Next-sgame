...
/* sgame-acepeek v20 — capture ALL sgame openat paths (not just /maps).
 *
 * v19 caught the smoking gun: ACE fires 21000+ openat in the 2s
 * before sgame dies, but our /maps filter sees zero of them. Whatever
 * file ACE is hammering is NOT /maps. v20 removes the /maps gate so
 * peek_ring records every sgame openat with its path, exposing the
 * real detection channel.
 *
 * Changes vs v19:
 *   - before_openat no longer requires "/maps" substring; logs all paths
 *   - PEEK_RING_SZ 32 -> 256 (last ~25ms at 10k/s spam rate)
 *   - PEEK_FN_LEN 64 -> 128 (longer paths like /proc/.../smaps_rollup)
 */

/* (kept for reference) sgame-acepeek v19 — auto-detect sgame process by task->comm.
 *
...
KPM_NAME("sgame-acepeek-v20");
KPM_VERSION("0.20.0");
KPM_LICENSE("GPL v2");
KPM_AUTHOR("ctf");
KPM_DESCRIPTION("v19 + capture ALL sgame openat paths (no /maps filter)");
...
#define PEEK_RING_SZ 256
#define PEEK_FN_LEN  128
...
/* before_openat — v20: peek EVERY sgame openat (no path filter).
 * Captures the file paths ACE spams in the 2s before SIGBUS to
 * identify the real detection channel beyond /proc/self/maps.
 */
static void before_openat(hook_fargs4_t *args, void *udata)
{
    ...
    /* v20: no /maps filter — record every openat path */
