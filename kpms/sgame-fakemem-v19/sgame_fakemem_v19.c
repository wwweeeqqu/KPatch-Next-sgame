...
static int line_is_suspicious(const char *line, int len)
{
    ...
    /* v18: 过滤 mountinfo 里我们 mount-bind 的 libtersafe.so 异常行 */
    if (str_contains(line, len, "libtersafe")) return 1;
    return 0;
}
...
static long sgs_init(const char *args, const char *event, void *__user reserved)
{
    ...
    pr_info("[fakemem-v19] fns: pidns=%llx cfu=%llx access=%llx\n",
    ...
    pr_info("[fakemem-v19] hooks: fchmodat=%d openat=%d read=%d close=%d faccessat=%d faccessat2=%d newfstatat=%d statx=%d\n",
...
static long sgs_exit(void *__user reserved)
{
    ...
    pr_info("[fakemem-v19] exit reads=%llu lines_removed=%llu hide_access=%llu hide_stat=%llu hide_open=%llu\n",
